...
class _SupportChatScreenState extends State<SupportChatScreen> {
  ...
  static const Color primary = Color(0xFF3b82f6); // Blue Support color
  ...
  void initState() {
    ...
    _messages.add({
      'text': "Hello! I am your **SpendWise Support Agent**.\n\nI am here to help you navigate the app, explain how features work, and troubleshoot any issues. How can I assist you today?",
      'isUser': false,
  ...
  Future<void> _sendMessage(String text) async {
    ...
    try {
      // Call Gemini using the Support service
      final responseText = await AiAdvisorService.getSupportAdvice(text, _chatHistory);
      ...
      setState(() {
        _isTyping = false;
        _messages.add({
          'text': "Sorry, I ran into an issue connecting to support: $e",
          'isUser': false,
          'timestamp': DateTime.now(),
        });
      });
          ...
          children: [
            Container(
              ...
              child: const CircleAvatar(
                ...
                radius: 18,
                child: Icon(Icons.support_agent, color: primary, size: 22),
              ),
            ),
          ...
          IconButton(
            ...
            onPressed: () {
              setState(() {
                ...
                _chatHistory.clear();
                _messages.add({
                  'text': "Chat history refreshed. How can I help you navigate SpendWise Sync today?",
                  'isUser': false,
                  'timestamp': DateTime.now(),
                });
              });
  ...
  Widget _buildMessageBubble({
    ...
    final bubbleBg = isUser ? primary.withValues(alpha: 0.15) : cardSurfaceColor;
    ...
    return Align(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isUser ? primary.withValues(alpha: 0.4) : borderColor,
          ),
        ),
        child: Column(
  ...
  Widget _buildSuggestionChips(Color textGrey, Color borderColor, Color cardSurfaceColor) {
    final chips = [
      {'label': '📊 View my budget', 'prompt': 'Where can I view my budget limits and progress?'},
      {'label': '🧾 Scan receipts', 'prompt': 'Can you explain how to use the camera to scan physical receipts?'},
      {'label': '📅 Calendar features', 'prompt': 'What features does the synced calendar provide?'},
      {'label': '📄 Export my data', 'prompt': 'How do I export my balance sheet as a PDF or CSV?'},
    ];
